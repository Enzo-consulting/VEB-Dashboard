...
## Correctif (2026-08-04) — rien ne persistait

**Cause** : dans `schema.sql`, la colonne `desc` de la table `annonces` n'était
pas entre guillemets. `DESC` est un mot réservé PostgreSQL → la requête
`CREATE TABLE annonces` échouait, ce qui annulait toute la transaction
(le script contient plusieurs `CREATE TABLE` envoyés en un seul bloc).
Résultat : **aucune table n'existait dans Supabase** (`clients`, `devis`,
`commandes` compris), donc chaque sauvegarde échouait silencieusement
(`.catch` côté frontend) et l'app retombait sur le `localStorage` du
navigateur — d'où l'impression que rien n'était mémorisé durablement.

**Fix** : colonne renommée `"desc"` (entre guillemets) dans `schema.sql`.
👉 Si ce n'est pas déjà fait, ré-exécutez tout `schema.sql` dans l'éditeur
SQL Supabase (il est idempotent : `CREATE TABLE IF NOT EXISTS`).

**Bonus corrigé en même temps** : `PUT /api/crm` ne faisait qu'un upsert et
ne supprimait jamais les lignes retirées côté client (suppression d'un
client/devis/commande côté UI ne se répercutait jamais dans Supabase).
`server.js` fait maintenant un vrai remplacement (delete des id absents +
upsert) via `replaceTable()`.
