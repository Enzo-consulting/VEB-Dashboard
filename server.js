...
// Dernière date de modification réelle des données (pas l'horloge serveur) :
// le frontend s'en sert pour détecter s'il faut resynchroniser depuis le cloud.
// Basé sur l'heure serveur (new Date()) ça change à chaque appel et déclenche
// une resynchronisation même quand rien n'a changé, ce qui écrasait des données
// locales pas encore reflétées côté Supabase — d'où le bug "un client ajouté
// disparaît après ~1 minute".
function maxUpdatedAt(...rowArrays) {
    let max = 0;
...
app.get('/api/crm', auth, withSupabase(async (req, res, ctx) => {
    ...
    if (commandesRes.error) throw new Error(commandesRes.error.message);
                                         ...
                                         res.json({
                                               ...
                                               _updatedAt: maxUpdatedAt(annoncesRes.data, clientsRes.data, devisRes.data, commandesRes.data)
                                         });
}));

// Remplace intégralement le contenu d'une table par la liste envoyée :
// supprime les lignes absentes de `rows`, puis upsert les lignes fournies.
// Nécessaire car le frontend envoie l'état complet à chaque sauvegarde
// (y compris après une suppression côté client).
async function replaceTable(admin, table, rows) {
    const ids = rows.map(r => String(r.id)).filter(Boolean);
    const { data: existing, error: selError } = await admin.from(table).select('id');
    ...
    if (toDelete.length) {
          const { error: delError } = await admin.from(table).delete().in('id', toDelete);
...
// PUT /api/crm — remplacement complet de toutes les collections (source de vérité = frontend)
app.put('/api/crm', auth, withSupabase(async (req, res, ctx) => {
    const { annonces = [], clients = [], devisList = [], commandesList = [] } = req.body || {};

                                         await replaceTable(ctx.supabaseAdmin, 'annonces', annonces);
    // Les devis/commandes référencent clients(id) — supprimer les clients d'abord
    // provoquerait un ON DELETE SET NULL, donc on synchronise clients avant devis/commandes.
    await replaceTable(ctx.supabaseAdmin, 'clients', clients);
    await replaceTable(ctx.supabaseAdmin, 'devis', devisList);
    await replaceTable(ctx.supabaseAdmin, 'commandes', commandesList);

                                         const [a2, c2, d2, cm2] = await Promise.all([
                                               ctx.supabaseAdmin.from('annonces').select('updated_at').order('updated_at', { ascending: false }).limit(1),
                                               ctx.supabaseAdmin.from('clients').select('updated_at').order('updated_at', { ascending: false }).limit(1),
                                               ctx.supabaseAdmin.from('devis').select('updated_at').order('updated_at', { ascending: false }).limit(1),
                                               ctx.supabaseAdmin.from('commandes').select('updated_at').order('updated_at', { ascending: false }).limit(1)
                                         ]);
                                         res.json({ ok: true, _updatedAt: maxUpdatedAt(a2.data, c2.data, d2.data, cm2.data) });
}));
